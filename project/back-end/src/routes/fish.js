...
router.put("/:id", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Fish updated successfully", fish: updatedFish });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error updating fish", error: error.message });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error deleting fish", error: error.message });
  }
});
